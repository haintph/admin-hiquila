    ...
    <div class="container-xxl">
        ...
        <div class="card">
                        ...
                        <tbody>
                            @forelse ($invoices as $invoice)
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge bg-info">Bàn {{ $invoice->table->table_number ?? 'Không xác định' }}</span>
                                    </td>
                                        ...
                                        <div class="d-flex gap-1">
                                            @if($invoice->status != 'Đã thanh toán')
                                                <a href="{{ route('invoices.edit', $invoice->invoice_id) }}" class="btn btn-warning btn-sm">
                                                    <iconify-icon icon="solar:pen-bold-duotone"></iconify-icon> Sửa
                                                </a>
                                                
                                                <a href="{{ route('invoices.checkout', $invoice->invoice_id) }}" 
                                                   class="btn btn-success btn-sm"
                                                   onclick="return confirm('Xác nhận thanh toán hóa đơn này?')">
                                                    <iconify-icon icon="solar:card-bold-duotone"></iconify-icon> Thanh toán
                                                </a>
                                                
                                                @if(config('app.qr_payment_enabled', true))
                                                    <a href="{{ route('invoices.payment', $invoice->invoice_id) }}" class="btn btn-info btn-sm">
                                                        <iconify-icon icon="solar:qr-code-bold-duotone"></iconify-icon> QR
                                                    </a>
                                                @endif
                                            @endif
                                            
                                            <a href="{{ route('invoices.print', $invoice->invoice_id) }}" class="btn btn-primary btn-sm">
                                                <iconify-icon icon="solar:printer-bold-duotone"></iconify-icon> In
                                            </a>
                                            
                                            <form action="{{ route('invoices.destroy', $invoice->invoice_id) }}" method="POST" class="d-inline">
                                                @csrf @method('DELETE')
                                                <button type="submit" class="btn btn-danger btn-sm" 
                                                        onclick="return confirm('Bạn có chắc chắn muốn xóa hóa đơn này?')">
                                                    <iconify-icon icon="solar:trash-bin-trash-bold-duotone"></iconify-icon> Xóa
                                                </button>
                                            </form>
                                        </div>
                                    </td>
            ...
            <div class="card-footer border-top d-flex justify-content-between align-items-center">
                <div>Hiển thị {{ $invoices->firstItem() ?? 0 }} đến {{ $invoices->lastItem() ?? 0 }} của {{ $invoices->total() ?? 0 }} hóa đơn</div>
                {{ $invoices->links() }}
            </div>
        </div>
    </div>
