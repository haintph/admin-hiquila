...
class StaffController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('staff.invoices.edit', $invoice->invoice_id)
    ...
    public function payment($id)
    {
        ...
        if (!in_array($invoice->status, ['Đã phục vụ', 'Đã thanh toán'])) {
            return redirect()->route('staff.invoices.edit', $id)
        ...
        if ($invoice->status === 'Đã thanh toán') {
            return view('staff.index', compact('invoice'));
        }
    ...
    public function checkout($id)
    {
        ...
        try {
            ...
            return redirect()->route('staff.index', $invoice->invoice_id)
    ...
    public function confirmPayment(Request $request, $id)
    {
        ...
        try {
            ...
            return redirect()->route('staff.invoices.payment', $invoice->invoice_id)
    ...
    public function dash(Request $request)
    {
        try {
            ...
            return view('staff.reservations', compact('reservations', 'stats', 'areas'));
        } catch (\Exception $e) {
    ...
    public function show($tableId)
    {
        try {
            ...
            return view('staff.show', compact('table'));
        } catch (\Exception $e) {
    ...
    public function vnpayReturn(Request $request)
    {
        if ($request->vnp_ResponseCode == '00') {
            ...
            try {
                ...
                return redirect()->route('staff.index')->with('success', 'Thanh toán VNPAY thành công!');
            } catch (\Exception $e) {
                DB::rollBack();
                \Illuminate\Support\Facades\Log::error('Lỗi khi cập nhật sau thanh toán VNPAY: ' . $e->getMessage());
                return redirect()->route('staff.index')->with('error', 'Thanh toán thành công, nhưng có lỗi khi cập nhật: ' . $e->getMessage());
            }
        }

        return redirect()->route('staff.index')->with('error', 'Thanh toán VNPAY thất bại!');
    }
}
